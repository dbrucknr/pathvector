...
| Clause | Confidence | Notes | What would close this out |
|---|---|---|---|
| Unrecognized **well-known** attribute (Optional bit = 0, but the type code itself isn't recognized) → distinct from an ordinary unrecognized optional attribute, which is meant to be accepted | **Confirmed gap. Fixed 2026-07-31 — see TODO.md #14** (`fix/rfc4271-unrecognized-optional-parameter-and-attribute`), **policy revised 2026-08-03** (`fix/rfc4271-unrecognized-well-known-attribute-treat-as-withdraw`). | `decode_path_attributes` checks the Optional bit for any type code `expected_optional_transitive` doesn't recognize: Optional bit set still decodes cleanly as `PathAttribute::Unknown` (unchanged, per §5); Optional bit clear produces an `AttributeErrorPolicy::TreatAsWithdraw` error. The first fix (2026-07-31) implemented the literal RFC 4271 §6.3 text — NOTIFICATION(UPDATE Error, Unrecognized Well-known Attribute) + full session reset, since RFC 7606's §3 revisions are an explicit, exhaustive (a)-(j) list that never mentions this subcode. The follow-up (2026-08-03) revised this to treat-as-withdraw after checking a real-world implementation (BIRD's `bgp_decode_attrs`, `proto/bgp/attrs.c`) rather than relying on the literal text alone: BIRD uses its `WITHDRAW` path (session stays up), not `REJECT` (session reset), for exactly this case — extending RFC 7606's "minimize blast radius" philosophy to a clause it doesn't explicitly cover. This project now follows that same practice. | `test_unrecognized_well_known_attribute_is_treat_as_withdraw` |
