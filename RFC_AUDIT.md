...
| Length field MUST have the smallest value required — "padding of extra data after the message is not allowed" | **Confirmed gap. Fixed 2026-08-03 — see TODO.md #12** (`fix/rfc4271-message-padding-rejection`). | `decode_with_limit_and_caps`'s `Open` and `RouteRefresh` match arms now each check `cur.remaining() != 0` immediately after their respective decode call, mirroring the pre-existing `Keepalive` check, returning `CodecError::InvalidLength(total_len)` if trailing bytes remain. Confirmed via direct RFC 7606/RFC 8654 text checks that neither amends this clause (no mention of "padding" in either). `Update` and `Notification` remain correctly unaffected, as already noted here: both are required by the RFC itself to consume the rest of the message. | `test_open_with_trailing_padding_is_error`, `test_open_with_trailing_padding_after_capabilities_is_error`, `test_route_refresh_with_trailing_padding_is_error` |

**Summary so far:** 4 clauses reviewed — 4 confirmed correct (1 after a fix).
...
| Clause | Confidence | Notes | What would close this out |
|---|---|---|---|
| KEEPALIVE consists of only the message header, no body (implies Length MUST always be exactly 19 for this type) | Confirmed correct | `decode_with_limit`'s `Keepalive` arm explicitly checks `cur.remaining() != 0` and errors — `Open` and `RouteRefresh` gained the identical check per the §4.1 fix above (2026-08-03) | `test_decode_header_keepalive`, `test_encode_keepalive_produces_19_bytes` |
...
| Unrecognized transitive optional attributes SHOULD be accepted and MUST be passed to other peers with Partial bit set to 1; unrecognized non-transitive attributes MUST be quietly ignored and not passed along | **Confirmed gap. Fixed 2026-08-03 — see TODO.md #13** (`feature/rfc4271-unrecognized-transitive-attribute-storage`). | `Route<A>`'s `RareAttrs` companion had no field of any kind for an opaque/unrecognized attribute, so any unrecognized attribute — transitive or not — was unconditionally dropped the moment a route was accepted into the RIB. Checked what a real-world implementation does before scoping the fix: BIRD (`proto/bgp/attrs.c`) already stores unknown attributes generically as opaque bytes and re-exports transitive ones with Partial set — confirming this was "just" a data-model gap. Added `pathvector_types::UnknownAttribute { type_code, value }` (no dependency edge needed between `pathvector-rib` and `pathvector-session` — both already depend on `pathvector-types`); `RareAttrs.unknown: Vec<UnknownAttribute>` follows the existing lazy-allocate pattern; `handle_update` stores only Optional=1,Transitive=1 attributes (quietly drops the rest, per this exact clause); `route_to_attributes`/`route_v6_to_attributes` forward them unconditionally regardless of peer type (like OTC). Note the *existing* `pathvector-session/RFC.md` row "Unknown optional transitive attributes preserved in Partial flag ✅" was accurate only at the codec round-trip level and never claimed to cover the RIB-pipeline path this finding was about — that distinction is now moot since the RIB pipeline is fixed too. | `test_handle_update_stores_unrecognized_transitive_attribute`, `test_handle_update_quietly_ignores_unrecognized_non_transitive_attribute`, `unrecognized_transitive_attribute_is_forwarded_with_optional_transitive_flags` |
| LOCAL_PREF received from an external (eBGP) peer MUST be ignored by the receiving speaker (§5.1.5) — distinct from the already-implemented "MUST NOT send LOCAL_PREF to eBGP peers" outbound rule | **Confirmed gap — highest severity finding so far. Fixed 2026-07-18 — see TODO.md #13** (`fix/rfc4271-local-pref-ebgp-ignore`). The RFC 4271 §5.1.5 confederation exception ("except in the case of BGP Confederations [RFC3065]") is deliberately not handled — see TODO.md item #22 for why. | `handle_update`'s attribute loop (`pathvectord/src/daemon/route.rs:944-964`) captures `PathAttribute::LocalPref(lp)` into a local variable with no `peer_type` check at all, and it's applied to the built route unconditionally (line 1154-1156: `if let Some(lp) = local_pref { builder = builder.local_pref(lp); }`). `RouteBuilder::build()` (`pathvector-rib/src/route.rs:409-422`) does not strip it based on peer type either — pure passthrough. `best_path.rs:167-169`'s comparator reads `.local_pref` uniformly for every route regardless of where it came from. **Practical impact:** an eBGP peer can attach an arbitrary LOCAL_PREF (e.g. `u32::MAX`) to a route it sends us, and since LOCAL_PREF is the *first* tie-break step in the decision process (RFC 4271 §9.1.1, ahead of AS_PATH length, ORIGIN, MED, etc.), that peer can force its own route to win best-path selection against routes we'd otherwise prefer — exactly the outcome this MUST exists to prevent. This is the class of bug the RFC calls out by name as a reason for the rule, not a theoretical edge case. `pathvectord/RFC.md` was updated to add this as a ❌ row (previously not tracked at all — the existing rows only covered the *outbound*, eBGP-peer-facing side of LOCAL_PREF handling). | A test sending an UPDATE from an eBGP-typed peer with an explicit LOCAL_PREF attribute, asserting the resulting route's `local_pref` is `None` (falls back to default) rather than the peer-supplied value — then gate the `PathAttribute::LocalPref` match arm (or the `builder.local_pref(lp)` call) on `peer_type == PeerType::Internal`. |
...
**§5 summary:** 11 clauses reviewed — 9 confirmed correct (2 of those as
deliberate, non-gap scope choices; 2 fixed after being confirmed gaps, one
of which was this audit's most severe finding to date), 2 needs
investigation.
...
| Unrecognized Optional Parameter (parameter type, not capability code) → NOTIFICATION(OPEN Error, Unsupported Optional Parameters) | **Confirmed gap. Fixed 2026-07-31 — see TODO.md #14** (`fix/rfc4271-unrecognized-optional-parameter-and-attribute`). | `decode_capabilities` now returns `CodecError::UnsupportedOptionalParameter { param_type }` for any Parameter Type other than 2, mapped to NOTIFICATION(OPEN Error, `OpenMsgError::UnsupportedOptionalParameter`) in `header_error_notification`. Confirmed via direct RFC 5492 read that it doesn't revise this clause — it only adds the separate, finer-grained Unsupported Capability subcode for an individual capability inside an already-recognized Capabilities parameter. | `test_unknown_opt_param_type_is_rejected`, `test_authentication_opt_param_type_1_is_rejected`, `test_unrecognized_opt_param_after_valid_capabilities_still_rejected`, `test_unsupported_optional_parameter_sends_open_message_notification` |
...
| Clause | Confidence | Notes | What would close this out |
|---|---|---|---|
| Unrecognized **well-known** attribute (Optional bit = 0, but the type code itself isn't recognized) → NOTIFICATION(UPDATE Error, Unrecognized Well-known Attribute) — distinct from an ordinary unrecognized optional attribute, which is meant to be accepted | **Confirmed gap. Fixed 2026-07-31 — see TODO.md #14** (`fix/rfc4271-unrecognized-optional-parameter-and-attribute`). A 2026-08-03 follow-up briefly revised this to treat-as-withdraw to match BIRD's practice, then **reverted back to session-reset 2026-08-04** after code review flagged it as a compliance regression — see the note below. | `decode_path_attributes` now checks the Optional bit for any type code `expected_optional_transitive` doesn't recognize: Optional bit set still decodes cleanly as `PathAttribute::Unknown` (unchanged, per §5); Optional bit clear produces an `AttributeErrorPolicy::SessionReset` error carrying `UpdateMsgError::UnrecognizedWellKnownAttribute` and a Data field of (type, length, value), per §6.3's exact text. Confirmed via direct RFC 7606 read that its §3 revisions are an explicit, exhaustive (a)-(j) list that never mentions this subcode, so it remains a full RFC 4271 session reset rather than treat-as-withdraw. **On the 2026-08-03→08-04 back-and-forth:** checking BIRD's source (`bgp_decode_attrs`, `proto/bgp/attrs.c`) found it uses `WITHDRAW` (session stays up) here, not `REJECT` — and this project briefly switched to match it, on the reasoning that BIRD extends RFC 7606's blast-radius-minimization philosophy to a clause RFC 7606 doesn't explicitly cover. A code review (Codex) on that PR correctly flagged this as a compliance regression: §6.3 is an unamended MUST, not an area the RFC leaves ambiguous, so "a specific other implementation does X" isn't a valid basis for deviating from unambiguous RFC text absent an actual amending RFC. Reverted to literal-RFC-4271 session-reset; BIRD's differing practice is noted here for reference only, not followed. This project still imitates BIRD for genuinely underspecified/ambiguous areas (e.g. RFC 4724 §4.1's Restarting-Speaker deferral scope) — the distinction is whether the RFC text itself leaves room for interpretation. | `test_unrecognized_well_known_attribute_is_session_reset`, `test_unrecognized_well_known_attribute_extended_length_data_field`, `test_unrecognized_well_known_attribute_sends_correct_notification_and_terminates` |
...
| A BGP speaker MUST NOT include more than one instance of the Graceful Restart Capability; if the peer does anyway, the receiver **MUST ignore all but the *last* instance** | **Confirmed gap. Fixed 2026-07-18 — see TODO.md #17** (`fix/rfc4724-rfc9234-capability-duplicates`). | `peer.rs:383-402`'s `.find_map(\|c\| ...)` iterates the peer's advertised capabilities in wire order and returns the **first** `GracefulRestart` capability with `restart_time > 0` — the opposite of "ignore all but the last." The existing tests (`duplicate_gr_capabilities_do_not_panic_and_first_wins`, `zero_gr_then_nonzero_gr_uses_first_nonzero`) directly name and assert the current (non-compliant) "first wins" behavior. Real-world likelihood is low — a peer sending 2+ GR capability instances is itself an RFC violation on the sender's part — but the receiver-side handling is backwards when it happens. Corrected the misleading `pathvectord/RFC.md` row (which had claimed this as ✅) to ⚠️. | Reverse the `find_map` to take the last match instead of the first (e.g. iterate and overwrite rather than short-circuit on first `Some`), then rename the tests to describe correct ("last wins") behavior |
...
| **MUST defer route selection for an address family until (a) EOR received from all GR-capable peers (excluding restarting ones) or (b) the Selection_Deferral_Timer expires; an implementation MUST support a configurable timer for this** | **Fixed — see `daemon/deferral.rs`, `selection_deferral_time` config.** Scoped to **outbound-advertisement-only** deferral (Loc-RIB/FIB computation stays immediate) rather than the RFC's literal "defer route selection" — this matches BIRD's real implementation, confirmed by reading `nest/proto.c`'s doc comment directly: BIRD "implements graceful restart recovery by deferring export of routes to protocols until routing tables are refilled," not by deferring Loc-RIB computation itself. Applies on **every** startup when configured nonzero, independent of the `restarting` flag (which only controls the R-bit) — a startup with incomplete peer state is a risk regardless of whether the restart was planned. | `SelectionDeferral` struct (`daemon/deferral.rs`): per-family (`v4_released`/`v6_released`) one-way latch, `recompute()` implements the wait-set over the **full configured peer set** (not just currently-established peers — an early design draft got this wrong and was caught in review: at daemon startup zero peers are established yet, so an established-only wait-set would let one fast peer's EOR falsely satisfy the wait-set before a slow peer even attempts its TCP connection), `force_release()` implements timer-expiry path (b). `propagate_prefix`/`propagate_prefix_v6` (`outbound.rs`) gained a `deferred: bool` parameter — when deferred, returns `NoChange` before touching `loc_rib`/`adj_rib_out`/`export_policy` at all, so there's nothing to reconcile once the gate opens. `on_established`'s initial dump is independently gated per family; `recompute_selection_deferral` (triggered on EOR receipt, peer establishment, and permanent peer removal) runs a gate-open catch-up (full dump + EOR) for every currently-established peer once a family releases. **Wait-set membership correction (Codex PR review, 2026-08-04):** §3 explicitly recommends advertising GracefulRestart with `restart_time == 0` (no `<AFI,SAFI>` families) purely to signal EOR intent, and §4.1's exclusion is for peers that "do not advertise the graceful restart capability" at all — not for `restart_time == 0` peers. The first cut of `extract_gr_capability()` collapsed `restart_time == 0` to the same "no capability" bucket used for `gr_capable_peers` (the stale-route-retention set, correctly `restart_time > 0`-gated), so an EOR-only peer was wrongly excluded from the wait-set and released the gate before its EOR arrived — exactly the incomplete-RIB startup churn this feature exists to prevent. Fixed by tracking "advertised GR capability at all" (`RibSnapshot::gr_advertised_peers`, new field) separately from `gr_capable_peers`; `extract_gr_capability()` now returns a `bool` advertised flag alongside `restart_time`, and extracts the R-bit regardless of `restart_time` (previously lost for the zero-time case, `gr_peer_restarting` was wrongly never set for an EOR-only-but-restarting peer either). | `daemon/deferral.rs` unit tests (wait-set membership rules, one-way latch, force-release), `outbound.rs` deferred-gate tests (NoChange + AdjRibOut untouched for both Announce- and Withdraw-shaped decisions), `daemon/mod.rs::selection_deferral_tests` (gate blocks propagation until EOR then catches up; unestablished configured peer keeps the gate closed; **an EOR-only (`restart_time == 0`) GR peer keeps the gate closed until its own EOR**; forced release overrides an unsatisfied wait-set; `selection_deferral_time == 0` is a true no-op). |

**§4.1 summary:** 4 clauses reviewed — 4 confirmed correct (the former gap
is now fixed).
...
21 clauses reviewed — 21 confirmed correct. **RFC 4724 audit considered
complete.** The Receiving Speaker role (§4.2) — the half most directly
exercised by this project's own restart scenarios in practice (peers
restarting, not `pathvectord` itself) — was always genuinely solid. The 3
originally-confirmed gaps have all since been fixed: duplicate-capability
first-vs-last handling (`TODO.md` #17/#18), the Established-collision GR
override (`TODO.md` #17, directly tied to the RFC 4271 §6.8 finding), and
the Restarting-Speaker Selection_Deferral_Timer (this pass — see the §4.1
table above for the scoping decisions and implementation).
...
| §5: Data field of the Unsupported Capability NOTIFICATION MUST encode each capability "in the same way as it would be encoded in the OPEN message" | **Confirmed gap. Fixed 2026-07-30 — see TODO.md #20** (`fix/rfc5492-unsupported-capability-notification-data`, GH PR #43). Checked whether RFC 8810 (the only RFC that updates RFC 5492) revises this clause — it doesn't; RFC 8810 only revises Capability Code IANA registration ranges, unrelated to error handling. | `encode_unsupported_capabilities` (`fsm/mod.rs`) now reuses `open::encode_capability_value` to build the real `code(1) + length(1) + value` TLV for each capability, matching the OPEN-message encoding exactly, instead of the old `[code, 0x00]` placeholder that omitted the value. Covered by `test_unsupported_capability_notification_encodes_full_capability_tlv`, which asserts the exact TLV bytes for a rejected `MultiProtocol(AFI=2,SAFI=1)` capability — deliberately not `RouteRefresh` (the pre-existing test's capability), since its zero-length value can't distinguish the old placeholder from the correct encoding. | Closed — see fix reference in the Confidence column |
