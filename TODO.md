...
**C. No e2e test for route reflection** — all 13 RFC 4456 tests are unit tests driving `DaemonState` directly. There is no end-to-end test that sets up a multi-peer RR topology (pathvectord as RR, two GoBGP peers — one client, one non-client) and verifies that routes are reflected correctly at the wire level. Unit tests prove the state machine is right; an e2e test is needed to prove wire behavior and interoperability. Add to `e2e/tests/` using the same `GobgpHarness` pattern; requires a three-container compose (gobgp-client, pathvectord-rr, gobgp-non-client).
...
- **`reapply_import_policy` has no IPv6 counterpart** — `reapply_import_policy` re-evaluates import policy for all routes in `AdjRibIn` of the target peer, but only for the IPv4 RIB. A policy reload on a peer that has IPv6 routes silently skips re-evaluation for those routes; IPv6 routes continue to be forwarded under the old policy until the session is reset. Fix: add `reapply_import_policy_v6` (parallel to the existing function) and call both in `SoftReset` and any future policy-reload path.

- **`cluster_id` configuration guidance** — the default `cluster_id = u32::from_be_bytes(local_bgp_id.octets())` is RFC 4456-correct, but the config docs do not explain that distinct `cluster_id` values are required when running multiple independent RR clusters. Without explicit configuration, loop detection via CLUSTER_LIST will behave unexpectedly if clusters share a `cluster_id`. Document in `pathvectord/README.md` with an explicit "if you run multiple RR clusters, set distinct `cluster_id` values" callout alongside the `is_route_reflector` config example.
