...
mod tests {
    ...
    async fn stub_client_returns_correct_values() {
        ...
        assert!(c.add_peer(AddPeerParams {
        ...
        assert!(c.remove_peer(IpAddr::V4(Ipv4Addr::new(10, 0, 0, 1))).await.is_ok());
        assert!(c.soft_reset(IpAddr::V4(Ipv4Addr::new(10, 0, 0, 1)), "ipv4").await.is_ok());
    ...
    async fn daemon_client_impl_exercises_each_method() {
        ...
        assert!(matches!(
            c.soft_reset(IpAddr::V4(Ipv4Addr::new(10, 0, 0, 1)), "ipv4").await,
            Err(ClientError::Rpc(_))
        ));
    }
    ...
    async fn list_all_routes_callable_through_trait_and_tracks_calls() {
        ...
        impl DaemonClient for CountingClient {
            async fn list_peers(&mut self) -> Result<Vec<PeerState>, ClientError> { Ok(vec![]) }
            ...
            async fn list_all_routes(&mut self, _: Option<IpAddr>) -> Result<Vec<Route>, ClientError> {
                *self.call_count.lock().unwrap() += 1;
                Ok(vec![])
            }
            async fn get_best_route(&mut self, _: &str) -> Result<Option<Route>, ClientError> { Ok(None) }
            async fn list_candidates(&mut self, _: &str) -> Result<Vec<Route>, ClientError> { Ok(vec![]) }
            async fn set_import_default(&mut self, _: &str, _: bool) -> Result<(), ClientError> { Ok(()) }
            ...
            async fn add_peer(&mut self, _: AddPeerParams) -> Result<(), ClientError> { Ok(()) }
            async fn remove_peer(&mut self, _: IpAddr) -> Result<(), ClientError> { Ok(()) }
            async fn soft_reset(&mut self, _: IpAddr, _: &str) -> Result<(), ClientError> { Ok(()) }
        }

        let call_count = Arc::new(Mutex::new(0u32));
        let mut client = CountingClient { call_count: Arc::clone(&call_count) };

        let routes = client.list_all_routes(None).await.unwrap();
        assert!(routes.is_empty());
        let routes = client.list_all_routes(Some(IpAddr::V4(Ipv4Addr::new(10, 0, 0, 1)))).await.unwrap();
        assert!(routes.is_empty());

        assert_eq!(*call_count.lock().unwrap(), 2, "list_all_routes must be called once per invocation");
    }
}
