...
fn decode_path_attributes(
    ...
    while cur.remaining() > 0 {
        ...
        if let Some((expect_optional, expect_transitive)) = expected_optional_transitive(type_code)
        {
            ...
            // RFC 4271 §6.3 literally requires NOTIFICATION(Unrecognized
            // Well-known Attribute) + session reset here, and RFC 7606
            // doesn't explicitly amend this clause (checked directly — its
            // §3 revisions are an exhaustive (a)-(j) list that never
            // mentions this subcode). Real-world implementations extend RFC
            // 7606's "minimize blast radius" philosophy to it anyway: BIRD's
            // `bgp_decode_attrs` (proto/bgp/attrs.c) uses its `WITHDRAW`
            // path (session stays up, NLRI withdrawn) rather than `REJECT`
            // (session reset) for exactly this case. This project follows
            // that same practice rather than the stricter literal-RFC-4271
            // reading — an ordinary peer sending one unrecognized attribute
            // (e.g. due to version skew or a vendor extension) shouldn't
            // tear down the whole session over it. The sender's Optional
            // bit (not our own recognition of the type code) is what tells
            // us whether they intended this as well-known; an unrecognized
...
mod tests {
    ...
    #[test]
    fn test_unrecognized_well_known_attribute_is_treat_as_withdraw() {
        // Type code 210 is unrecognized by this decoder. Optional bit clear
        // (flags=FLAG_TRANSITIVE only, i.e. the sender claims this is
        // well-known) is RFC 4271 §6.3's "Unrecognized Well-known
        // Attribute" case. RFC 7606 never explicitly amends this clause,
        // but this project follows real-world practice (e.g. BIRD's
        // `bgp_decode_attrs` treats this as treat-as-withdraw, not a
        // session reset) rather than the stricter literal-RFC-4271
        // reading, matching how RFC 7606 treats every other
        // attribute-level error it *does* cover. Not silent acceptance as
        // `PathAttribute::Unknown` the way an unrecognized *optional*
        // attribute is, either.
        ...
        assert!(
            treat_as_withdraw,
            "unrecognized well-known attribute must set treat_as_withdraw"
        );
