...
| RFC 4271 §6.3: an unrecognized well-known attribute (unrecognized type code, Optional bit = 0) → treat-as-withdraw, distinct from the ordinary unrecognized-optional-attribute case (Optional bit = 1), which decodes cleanly as `PathAttribute::Unknown` | `src/message/update.rs` | ✅ | `decode_path_attributes` checks the Optional bit for any unrecognized type code. **History:** the literal RFC 4271 §6.3 text (not amended by RFC 7606 — its §3 revisions are an explicit, exhaustive (a)-(j) list that never mentions this subcode) requires NOTIFICATION(UPDATE Error, Unrecognized Well-known Attribute) + full session reset; this was the initial fix, 2026-07-31 (`fix/rfc4271-unrecognized-optional-parameter-and-attribute`). Revised 2026-08-03 (`fix/rfc4271-unrecognized-well-known-attribute-treat-as-withdraw`) to treat-as-withdraw instead, after checking a real-world implementation rather than relying on the literal text alone: BIRD's `bgp_decode_attrs` (`proto/bgp/attrs.c`) uses its `WITHDRAW` path (session stays up), not `REJECT` (session reset), for exactly this case. `test_unrecognized_well_known_attribute_is_treat_as_withdraw` |
