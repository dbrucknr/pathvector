...
Property tests prove invariants hold for all *valid* inputs generated by a strategy. Fuzz testing
proves that *arbitrary* byte sequences — including ones no sane implementation would produce — never
cause a panic or memory error. These are complementary: property tests are RFC conformance evidence;
fuzz targets are the panic-safety story for the external attack surface.

The only code reachable from an untrusted remote peer is the session codec decode path. Every byte
received over TCP passes through `BgpCodec::decode` → `BgpMessage::decode`, so that is what the
...
| `session_message` | Patches the 2-byte length field to match input length, then calls `BgpMessage::decode` directly. This drives body-parsing for all five message types regardless of what the framing layer would do. |

All targets live in `fuzz/fuzz_targets/` at the workspace root. The `session_` prefix reserves
space for future targets from other crates (e.g. `bmp_` when `pathvector-bmp` grows a decode
path). The seed corpus in `fuzz/corpus/` pre-populates one valid example of each message type
(KEEPALIVE, OPEN, UPDATE, NOTIFICATION, ROUTE-REFRESH) so the fuzzer starts from real
message boundaries rather than discovering the `0xFF × 16` marker pattern from scratch.

### Nightly requirement and why it's tricky on macOS

`cargo fuzz` uses `-Zsanitizer=address`, which requires a nightly compiler. On macOS with Rust
installed via Homebrew, `cargo` is a standalone stable binary — not a rustup proxy — so it ignores
`rust-toolchain.toml` and always resolves to stable `rustc`.

Two env-var tricks that look like they should work but don't:

- **`CARGO=<nightly-path> cargo fuzz ...`** — ineffective because Homebrew's `cargo` overwrites
  `CARGO` with its own path before spawning subcommands.
- **`~/.rustup/toolchains/.../bin/cargo fuzz ...`** — ineffective because `cargo-fuzz 0.13.x`
  hardcodes `Command::new("cargo")` and never reads the `CARGO` env var at all.

**The only lever that works is `PATH`**: if the nightly toolchain's bin directory appears before
Homebrew's, the hardcoded `"cargo"` lookup resolves to nightly. The `Justfile` handles this
automatically using `rustup which --toolchain nightly cargo` to locate the right bin directory on
any platform.
...
Then all fuzz invocations are a single command from the **workspace root**:
...
The fuzzer saves interesting new inputs to `fuzz/corpus/` automatically. Commit a grown corpus
back to the repo so future runs (and CI) start from a richer baseline.

**Reproducing a crash:** if a target panics, the input is saved to
...
The fuzz crate declares its own `[workspace]` (required by `cargo fuzz`) so it is not part of the
main workspace. rust-analyzer therefore does not index it by default. The `.zed/settings.json` at
the project root adds it as a linked project, which gives full LSP support — completions,
go-to-definition, and inline errors — in the fuzz target files without any manual steps.

Other editors using rust-analyzer can achieve the same by adding
`"rust-analyzer.linkedProjects": ["fuzz/Cargo.toml"]` to their equivalent per-project settings
file.

### CI

The `fuzz` job in `.github/workflows/ci.yml` runs `just fuzz-smoke` on every push and pull
request. On Linux CI runners, `cargo` is the rustup shim and nightly is already in PATH after
`dtolnay/rust-toolchain@nightly`, so the PATH prepend in the Justfile is a no-op — the same
recipe works correctly on both platforms. If a target panics, the crash inputs are uploaded as a
GitHub Actions artifact named `fuzz-crashes`.
