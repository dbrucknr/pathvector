...
| §12: Error Code 2 ("No Data Available") is explicitly non-fatal — session should stay usable, router should retry with periodic Reset Queries — distinct from the other 8 error codes, all explicitly marked "(fatal): ... MUST cause the session to be dropped" | `src/client.rs` | ✅ | `error_code_2_no_data_available_retries_on_same_connection` (fixed 2026-07-19, `fix/rfc8210-error-code-2-no-data-available`) — a dedicated `sync_once` match arm for `error_code == 2` retries the same query on the same connection after `retry_interval`, instead of falling through to the generic fatal-error `Err` path every other code still uses |
