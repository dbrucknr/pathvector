...
| §3 duplicate GR capabilities from peer: no panic (✅) but uses the **first** non-zero instance, not the **last** (❌) | `src/daemon.rs` | ⚠️ | `duplicate_gr_capabilities_do_not_panic_and_first_wins`, `zero_gr_then_nonzero_gr_uses_first_nonzero` — **corrected 2026-07-16 by `RFC_AUDIT.md`**: RFC 4724 §3 says "the receiver of the advertisement MUST ignore all but the **last** instance of the Graceful Restart Capability" — `peer.rs:383-402`'s `.find_map(...)` takes the first `GracefulRestart` capability with `restart_time > 0`, which is the opposite of what the RFC requires. The existing tests' own names ("first_wins") describe the current, non-compliant behavior, not RFC-correct behavior. Low real-world likelihood of a peer actually sending 2+ instances (the RFC calls it a sender violation), but the receiver-side handling is still backwards when it does happen. |
...
| §4.1 MUST: Restarting Speaker defers its own route selection until EOR received from all GR-capable peers or a Selection_Deferral_Timer expires | — | ❌ | Added 2026-07-16 by `RFC_AUDIT.md` (RFC 4724 audit). No `Selection_Deferral_Timer` or equivalent exists anywhere in the codebase — `handle_update`/`select_best` run immediately per-UPDATE with no notion of "our own restart is still in progress, hold off on final decisions." This project only implements the *Receiving Speaker* half of §4 (holding a *peer's* stale routes) — the *Restarting Speaker* half (deferring *our own* decisions after *our* restart) has no code path at all. See `RFC_AUDIT.md`'s RFC 4724 section for the practical consequence. |

**Deferred:** §3 SHOULD: suppress GR capability when peer's restart_time = 0 (currently logged
as warning only). §4.2 (Receiving Speaker role) requirements are implemented and e2e
verified. §4.1 (Restarting Speaker's own deferred route selection) and the §5 FSM
collision-override for GR peers are **not** implemented — see the ❌ rows above and
`RFC_AUDIT.md`'s RFC 4724 section, added 2026-07-16.
...
| RFC 4724 §5: when GR capability (≥1 AFI/SAFI) was negotiated for an Established session, a new incoming connection (Event 16/17) must be treated as peer restart — retain routes, drop the *old* connection, move to Connect — not rejected as an ordinary collision | `pathvector-session/src/transport/mod.rs` | ❌ | Added 2026-07-16 by `RFC_AUDIT.md` (RFC 4724 audit). `handle_incoming_connection`'s `State::Established` arm unconditionally rejects the new incoming connection with no check for GR capability at all — the plain RFC 4271 §6.8 behavior (reject new, keep existing) is correct for non-GR sessions but RFC 4724 explicitly overrides it for GR-negotiated ones. See `RFC_AUDIT.md`'s RFC 4724 section. |
