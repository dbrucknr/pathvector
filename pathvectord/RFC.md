...
route selection) requirements are both implemented and tested. The §5 FSM collision-override
for GR peers is also implemented — see `RFC_AUDIT.md`'s RFC 4724 section.
...
| RFC 4724 §5: when GR capability (≥1 AFI/SAFI) was negotiated for an Established session, a new incoming connection (Event 16/17) must be treated as peer restart — retain routes, drop the *old* connection, move to Connect — not rejected as an ordinary collision | `pathvector-session/src/transport/mod.rs` | ✅ | Fixed 2026-07-16 (`fix/rfc4271-collision-resolution`, `TODO.md` #17). `handle_incoming_connection`'s `State::Established` arm now checks GR-negotiated status and retains routes/drops the old connection/moves to Connect for GR peers instead of rejecting. See `RFC_AUDIT.md`'s RFC 4724 §5 entry. |
...
| e2e (2026-08-05, round 2, Codex follow-up review): the above was unit-tested only — nothing proved the real wire codec on both ends produces the expected split: a confed segment surviving the downgraded wire AS_PATH toward a fellow `ConfedMember` peer (RFC 5065 §5.3) while AS4_PATH excludes it entirely | `pathvector-e2e/src/bin/mock_bgp_as4path_peer.rs`, `pathvector-e2e/tests/as4path_confed.rs` | ✅ | `as4_path_excludes_confed_segment_while_wire_as_path_keeps_it` (a four-byte-ASN-capable confed-source sends AS_PATH = [AS_CONFED_SEQUENCE, 4-byte ASN]; a two-byte-only fellow confed-member observer decodes pathvectord's re-advertised UPDATE and confirms the confed segment survives in wire AS_PATH alongside AS_TRANS substitution, while AS4_PATH carries the real ASN with the confed segment excluded) |
